...
<head>
    <link rel="stylesheet" href="../css/card_payment.css">
...
<body>
    ...
    <form id="paymentForm" method="POST" action="../actions/process_order.php">
        <div class="field-container">
            ...
            <input id="name" name="first_name" maxlength="20" type="text" required>
        </div>
        <div class="field-container">
            <label for="phone">Card Number</label>
            <input id="cardnumber" name="phone" type="text" required>
        </div>
        <div class="field-container">
            <label for="address">Exp Date</label>
            <input id="expirationdate" name="address" type="text" required>
        </div>
        <div class="field-container">
            <label for="payment_method">CVV</label>
            <input id="securitycode" name="payment_method" type="text" required>
        </div>
        <div class="field-container">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const cardExpiration = document.getElementById('expirationdate');
            const cardSecurity = document.getElementById('securitycode');
            ...
            cardName.addEventListener('input', function () {
                svgName.textContent = this.value.toUpperCase() || "JOHN DOE"; // Fallback to default if empty
                svgNameBack.textContent = this.value.toUpperCase() || "John Doe"; // Back of the card name
            });

            // Update Card Number
            cardNumber.addEventListener('input', function () {
                let cardNum = this.value.replace(/\D/g, ''); // Remove non-numeric characters
                if (cardNum.length > 16) cardNum = cardNum.slice(0, 16); // Max 16 digits
                svgNumber.textContent = cardNum || "0123 4567 8910 1112"; // Fallback if empty
                // Add spaces for every 4 digits
                svgNumber.textContent = svgNumber.textContent.replace(/(\d{4})/g, '$1 ').trim();
            });
            ...
            cardExpiration.addEventListener('input', function () {
                ...
                let year = expDate.slice(2, 4);
                svgExpire.textContent = `${month || "01"}/${year || "23"}`; // Fallback to default if empty
            });

            // Update Security Code (CVV)
            cardSecurity.addEventListener('input', function () {
                let securityCode = this.value.replace(/\D/g, ''); // Remove non-numeric characters
                if (securityCode.length > 3) securityCode = securityCode.slice(0, 3); // Max 3 digits
                svgSecurity.textContent = securityCode || "985"; // Fallback to default if empty
            });
        });
